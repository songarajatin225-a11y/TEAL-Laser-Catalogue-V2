...
auto-rotate, callout labels, reset, the original 2D optical schematic, and
expand to full screen (Esc to exit). Drag to orbit, scroll or pinch to zoom.
...
Run it before exporting. Alongside dangling references it now catches unknown
beam delivery, unparseable footprints and cycle times, a work height that leaves
no chamber, duplicate short codes (two platforms sharing a model designation),
